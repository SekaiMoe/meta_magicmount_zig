<script>
  ...
  async function save() {
    ...
    message = null;
    try {
      config.partitions = partitionsInput.split(',').map(s => s.trim()).filter(Boolean);
...
<div class="card" in:fade={{ duration: 180 }}>
  ...
  <div class="field">
    ...
    <div class="loglevel-switch">
      <button type="button" class="lv-btn {!config.verbose ? 'active' : ''}" on:click={() => config.verbose = false}>{$L.config.verboseOff}</button>
      <button type="button" class="lv-btn {config.verbose ? 'active' : ''}" on:click={() => config.verbose = true}>{$L.config.verboseOn}</button>
    </div>
  </div>
  ...
  <div class="field">
    ...
    <div class="loglevel-switch">
      <button type="button" class="lv-btn {!config.umount ? 'active' : ''}" on:click={() => config.umount = false}>{$L.config.umountOff}</button>
      <button type="button" class="lv-btn {config.umount ? 'active' : ''}" on:click={() => config.umount = true}>{$L.config.umountOn}</button>
    </div>
  </div>
  ...
  <div class="field">
    <label class="field-label" for="inp-mountsource">{$L.config.mountSource}</label>
    <input 
      id="inp-mountsource" 
      type="text" 
  ...
  <div class="field">
    <label class="field-label" for="inp-partitions">{$L.config.extPartitions}</label>
    <input 
      id="inp-partitions" 
      type="text" 
  ...
  <div class="actions">
    <button type="button" on:click={load} disabled={loading || saving}>{$L.config.reload}</button>
    <button type="button" class="primary" on:click={save} disabled={loading || saving}>
